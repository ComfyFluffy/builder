...
import { languages, type IRange, editor } from 'monaco-editor'
...
const completionItemProvider: languages.CompletionItemProvider = {
  provideCompletionItems: (model, position) => {
    ...
    const suggestions: languages.CompletionItem[] = completionItem(range)
    return { suggestions }
  }
}
...
export const register = () => {
  ...
  editor.defineTheme('myTransparentTheme', {
    base: 'vs',
    inherit: true,
    rules: [
  ...
  languages.setLanguageConfiguration('spx', LanguageConfig)

  // Match token and highlight
  languages.setMonarchTokensProvider('spx', MonarchTokensProviderConfig)
  // Code hint
  languages.registerCompletionItemProvider('spx', completionItemProvider)
  initFormat()
}
